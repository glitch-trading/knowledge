...
- Open a GitHub issue before fixes or additions. Use a feature branch by default.
- For `glitch-trading/knowledge`, direct commits and non-force pushes to `main` are permitted when the owner explicitly requests the respective action. This repository-specific exception overrides inherited branch-only guidance and the default in `CLAUDE.md`; verification, secret exclusion and the prohibition on force-pushing shared history still apply.
