...
A **Markov Chain** is a discrete-time stochastic process over a discrete state space that satisfies the [[Markov Property]]: the next state depends only on the current state, not the full history. It is the simplest non-trivial stochastic process and the building block for most regime-aware models in finance.
...
- **Time-homogeneity**: $\mathbf{P}$ is assumed constant over time. Often violated in practice (see [[Regime Switching]]).
...
- **Foundation for richer models.** [[Hidden Markov Models]], regime-switching GARCH, and dynamic-programming control problems (Bellman, HJB) all assume a Markov chain over some latent state.
...
- [[Markov Property]] — the abstract property this concrete object satisfies
...
- [[Dynamic Programming]] — Bellman recursion assumes a Markov state
- [[Random Walk]] — the simplest Markov chain
- [[Stationarity]] — related-but-different notion (statistical properties constant over time, vs. distribution of the chain converging)
