...
- Used in prediction market AMMs alongside [[amm|AMM]] mechanisms like [[constant-product-formula|Constant Product Formula]]
- Prices are implied probabilities — connects to [[conditional-probability|Conditional Probability]] and [[bayes-theorem|Bayes' Theorem]]
- Bounded loss connects to [[position-sizing|Position Sizing]] and risk management
- The softmax function appears in ML classifiers — same mathematical structure
- The cost function $C(\mathbf{q}) = b\log\sum_i e^{q_i/b}$ is a scaled log-partition function — the same object that appears in the [[maximum-entropy-principle|Maximum Entropy Principle]] derivation, and it generates the [[bregman-divergence|Bregman Divergence]] used for arbitrage-free price projection in [[combinatorial-prediction-markets|Combinatorial Prediction Markets]] (on the simplex this Bregman divergence reduces to KL)
- [[frank-wolfe-algorithm|Frank-Wolfe Algorithm]] (barrier variant) is the standard optimizer used to project onto the marginal polytope under LMSR pricing without explicit polytope enumeration
- [[shannon-entropy|Shannon Entropy]] and [[kl-divergence|KL Divergence]] are the natural units for measuring price-implied uncertainty and trading edge on the resulting markets
- [[prediction-markets|Prediction Markets]] — venue category where LMSR is the canonical AMM
