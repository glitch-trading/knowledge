...
- **Adverse selection framing**: IL is caused by arbitrageurs trading against the pool whenever the AMM price diverges from the true market price. This is [[adverse-selection|Adverse Selection]] — the informed traders (arbs) extract value from LPs (uninformed).
...
- [[amm|AMM]] — IL is the defining risk of providing liquidity to AMMs
- [[constant-product-formula|Constant Product Formula]] — IL formula is derived directly from the $x \cdot y = k$ invariant
- [[liquidity|Liquidity]] — LPs must weigh IL against fee income when deciding to provide liquidity
- [[adverse-selection|Adverse Selection]] — IL is the LP's cost of being adversely selected by informed arbitrageurs
