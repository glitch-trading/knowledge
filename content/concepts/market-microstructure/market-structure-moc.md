---
title: "Market Structure — Map of Content"
type: MOC
tags:
  - MOC
  - markets
---

## Fundamentals
- [[order-types|Order Types]]
- [[order-book|Order Book]]
- [[spread|Spread]]
- [[liquidity|Liquidity]]
- [[slippage|Slippage]]

## Market Making
- [[market-making|Market Making]]
- [[market-maker|Market Maker]]
- [[inventory-risk|Inventory Risk]]
- [[adverse-selection|Adverse Selection]]
- [[reservation-price|Reservation Price]]
- [[avellaneda-stoikov|Avellaneda-Stoikov]]

## Venues
- CEX (Binance, Bybit, Coinbase)
- [[amm|AMM]]
- [[constant-product-formula|Constant Product Formula]]
- [[curve-stableswap|Curve Stableswap]]
- [[impermanent-loss|Impermanent Loss]]
- [[prediction-markets|Prediction Markets]] (Polymarket, Kalshi, Azuro) — binary CLOB and LMSR-AMM venues

## Execution
- [[twap|TWAP]]
- [[vwap|VWAP]]
- [[market-impact|Market Impact]]
- [[market-profile|Market Profile]] (TPO — value area, POC, single prints)
- Iceberg orders
- Scaling into a position vs. single-clip entry
- [[leg-risk|Leg Risk]] — non-atomic multi-leg fills (cross-venue, multi-market, on-chain)
- Atomicity spectrum — native atomic / bundle-atomic / portfolio-non-atomic / cross-chain

## MEV (Level 4)
- [[mev|MEV]]
- [[frontrunning|Frontrunning]]
- [[backrunning|Backrunning]]
- [[sandwich-attack|Sandwich Attack]]
- [[liquidation-botting|Liquidation Botting]]

## Microstructure (Level 6)
- Order flow analysis
- Kyle's lambda
- Lead-lag relationships
- [[vpin|VPIN]] — Volume-Synchronized Probability of Informed Trading
- Effective spread (see [[spread|Spread]])
