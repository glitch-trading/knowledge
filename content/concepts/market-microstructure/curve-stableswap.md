...
The Curve Stableswap invariant is a hybrid between the constant sum formula ($x + y = k$) and the [[constant-product-formula|Constant Product Formula]] ($x \cdot y = k$). It is specifically designed for assets that should trade near parity — stablecoins (USDC/USDT), wrapped assets (ETH/stETH), etc.
...
- [[amm|AMM]] — Curve is a specialized AMM design for correlated assets
- [[constant-product-formula|Constant Product Formula]] — Curve's invariant reduces to constant product when $A = 0$
- [[impermanent-loss|Impermanent Loss]] — IL dynamics differ on Curve due to the flatter curve near parity
