...
- "Is this Sharpe statistically distinguishable from zero?" → its own test, see [[The Deflated Sharpe Ratio — Bailey & López de Prado]].
- "Is the spread between these two assets cointegrated?" → ADF test (see [[Cointegration]]).
- "Is yesterday's return predictive of today's?" → test on the [[Autocorrelation]] coefficient.

**The trap is multiple testing.** Run 100 independent strategy backtests at $\alpha = 0.05$ and you expect ~5 false positives even if none of the strategies have any edge. This is the central reason why naively reported backtest Sharpe ratios overstate live performance — see [[Overfitting]].
...
- **Deflated Sharpe ratio:** the principled version for backtest counts — see [[The Deflated Sharpe Ratio — Bailey & López de Prado]].
...
| Augmented Dickey-Fuller (ADF) | Is this series stationary? | [[Cointegration]], spread mean-reversion |
| Ljung-Box | Are residuals serially correlated? | Diagnosing regression / ARIMA fits |
| Jarque-Bera | Are returns normal? | Almost always rejects on financial data (see [[Fat Tails]]) |
...
- [[All of Statistics — Larry Wasserman]] — Chapters 10-11 cover hypothesis testing rigorously
- [[Introduction to Statistical Learning (ISLR)]] — Applied perspective
- [[The Deflated Sharpe Ratio — Bailey & López de Prado]] — Multiple-testing correction for strategies

## Connections

- [[Overfitting]] — Multiple testing without correction is the most common path to overfit conclusions
- [[Regression]] — t-tests on coefficients underpin most factor model claims
- [[Cointegration]] — ADF is the test that makes pairs trading rigorous
- [[The Deflated Sharpe Ratio — Bailey & López de Prado]] — Strategy-specific multiple-testing correction
- [[Look-Ahead Bias]] — Even a correctly designed test gives wrong answers if inputs use future information
