...
- **THE concept for pairs/stat-arb trading**: Cointegration is the theoretical foundation for [[statistical-arbitrage|Statistical Arbitrage]]. If two assets are cointegrated, their spread $Z_t$ is mean-reverting — buy when the spread is wide, sell when it narrows.
...
- [[statistical-arbitrage|Statistical Arbitrage]] — Cointegration is the theoretical backbone of stat arb
- [[stationarity|Stationarity]] — The spread must be stationary for the relationship to be tradeable
- [[time-series-analysis|Time Series Analysis]] — Cointegration is a fundamental concept in multivariate time series
- [[regression|Regression]] — Engle-Granger uses regression as its first step (but with non-standard inference)
- [[arima|ARIMA]] — The spread can be modeled as an ARMA process for forecasting
