...
- **Return modeling**: Choosing the right distribution for asset returns determines everything — risk estimates, option prices, portfolio weights. The [[normal-distribution|Normal Distribution]] is the starting point, but real markets have [[fat-tails|Fat Tails]].
- **Order flow modeling**: The [[poisson-distribution|Poisson Distribution]] models order arrival counts; the [[exponential-distribution|Exponential Distribution]] models times between orders. Both are central to market-making models like Avellaneda-Stoikov.
...
- **Price modeling**: Asset prices (not returns) tend to follow the [[log-normal-distribution|Log-Normal Distribution]], which ensures prices stay positive.
- **Extreme events**: [[power-law-distribution|Power Law Distribution]]s capture the heavy tails seen in real market data — large moves happen far more often than a normal distribution predicts.
...
- [[normal-distribution|Normal Distribution]] — the most common continuous distribution, foundation of classical finance
- [[log-normal-distribution|Log-Normal Distribution]] — models asset prices (positive support)
- [[exponential-distribution|Exponential Distribution]] — models inter-arrival times
- [[poisson-distribution|Poisson Distribution]] — models event counts in fixed intervals
- [[power-law-distribution|Power Law Distribution]] — models heavy-tailed phenomena like large trades and crashes
