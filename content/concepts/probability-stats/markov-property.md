...
- **Dynamic programming**: The [[dynamic-programming|Dynamic Programming]] approach (Bellman equation) requires the Markov property. The optimal strategy at time $t$ depends only on the current state $(S_t, q_t, t)$, not the history. The Avellaneda-Stoikov HJB equation is built on this assumption.
...
- [[markov-chain|Markov Chain]] — the discrete-time, discrete-state Markov process: transition matrix mechanics, $n$-step forecasts, stationary distribution
- [[random-walk|Random Walk]] — the simplest Markov process
- [[brownian-motion|Brownian Motion]] — a continuous-state, continuous-time Markov process
- [[dynamic-programming|Dynamic Programming]] — requires the Markov property to define value functions
