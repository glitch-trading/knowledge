...
- **Inter-arrival times**: In the [[poisson-process|Poisson Process]], the time between consecutive events is exponentially distributed. If orders arrive at rate $\lambda$ per second, the time to the next order is $\text{Exp}(\lambda)$.
...
- [[poisson-process|Poisson Process]] — exponential inter-arrival times define a Poisson process
- [[poisson-distribution|Poisson Distribution]] — the Poisson distribution counts events; the exponential models the gaps between them
