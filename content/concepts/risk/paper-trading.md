...
- **Connection / reconnection logic.** Websocket disconnects, exponential-backoff behavior, stale-data detection, failover to secondary feeds. See [[trading-bot-operations|Trading Bot Operations]].
...
- **Adverse selection.** Your simulated fills are filled by no one and selected by nothing; live limit-order fills disproportionately occur when the price is about to move against you. See [[adverse-selection|Adverse Selection]].
...
- **Feedback effects** — at any size, your orders move the book; this matters even before classical [[market-impact|Market Impact]] kicks in.
...
**Paper trading as ongoing canary.** After going live, keep a parallel paper instance running. Real-vs-paper divergence beyond a calibrated threshold is the cleanest early signal of an unmodeled cost (slippage drift, adverse selection ramping), an [[adverse-selection|Adverse Selection]] regime change, or a silent live-pipeline bug. Treat divergence the same as any kill-switch trigger — halt, diagnose, restart only with a documented fix. See [[kill-switch|Kill Switch]].
...
- [[advances-in-financial-machine-learning-lopez-de-prado|Advances in Financial Machine Learning — López de Prado]] — backtest-vs-live methodology and overfitting controls

## Connections

- [[trading-bot-operations|Trading Bot Operations]] — paper trading is the validation layer in the production checklist
- [[kill-switch|Kill Switch]] — paper mode is also where kill-switch logic is exercised and tuned
- [[overfitting|Overfitting]] — paper mode catches some out-of-sample drift the backtest didn't
- [[trade-journaling|Trade Journaling]] — paper trades feed the same journal pipeline as live trades
- [[adverse-selection|Adverse Selection]] — the dominant unmodeled cost paper trading misses
- [[market-impact|Market Impact]] / [[slippage|Slippage]] — the second category of unmodeled cost
- [[leg-risk|Leg Risk]] — non-atomic multi-leg fills are paper-mode-invisible unless explicitly simulated
