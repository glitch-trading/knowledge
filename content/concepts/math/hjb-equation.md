...
## What It Is
The Hamilton-Jacobi-Bellman (HJB) equation is the continuous-time analog of the [[bellman-equation|Bellman Equation]]. It is a partial differential equation (PDE) that the optimal value function must satisfy. Where the Bellman equation uses discrete "max over actions," the HJB equation uses a supremum over continuous controls and replaces sums with derivatives.
...
- Solving the HJB gives the optimal quoting policy — this is where the [[reservation-price|Reservation Price]] formula comes from
- The [[feynman-kac-theorem|Feynman-Kac Theorem]] connects the HJB PDE back to the stochastic expected-utility problem
...
## Connections
- [[bellman-equation|Bellman Equation]] — the HJB is the continuous-time limit of the Bellman equation
- [[dynamic-programming|Dynamic Programming]] — HJB is the continuous-time dynamic programming principle
- [[feynman-kac-theorem|Feynman-Kac Theorem]] — connects the HJB PDE to expected values of stochastic processes
- [[avellaneda-stoikov|Avellaneda-Stoikov]] — the HJB equation is the core mathematical tool in the paper
