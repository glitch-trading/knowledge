...
- **Understanding the HJB equation**: The [[hjb-equation|HJB Equation]] is the continuous-time limit of the Bellman equation. Understanding the discrete version first makes the continuous version intuitive — the "max over actions" and "expected future value" structure carry over directly
...
For continuous state/action spaces, this same loop becomes the [[hjb-equation|HJB Equation]] in the $\Delta t \to 0$ limit, with the `max` replaced by a `sup` over a control set.
...
## Connections
- [[dynamic-programming|Dynamic Programming]] — the Bellman equation is the core equation of dynamic programming
- [[hjb-equation|HJB Equation]] — the continuous-time generalization of the Bellman equation
