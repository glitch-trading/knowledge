---
title: "Level 2 — Basic Probabilistic Arbitrage"
type: roadmap
level: 2
status: not-started
tags:
...
How a [[random-walk|Random Walk]] with smaller steps converges to [[brownian-motion|Brownian Motion]]. What `dW` means — an infinitesimal random increment. What `dS = σdW` means — price moves randomly with volatility σ. [[geometric-brownian-motion|Geometric Brownian Motion]] `dS/S = μdt + σdW` — the standard stock price model.

#### [[ito-calculus|Itô Calculus]]
Normal calculus chain rule fails because [[brownian-motion|Brownian Motion]] is "rough." The single most important fact in quantitative finance: `(dW_t)² = dt`. This is why [[itos-lemma|Itô's Lemma]] has an extra second-order term that doesn't appear in ordinary calculus. Given `f(S,t)` where S follows an SDE, compute `df`. Practice this mechanically — it's a tool you'll use constantly.
...
[[feynman-kac-theorem|Feynman-Kac Theorem]]: expected values of functions of stochastic processes satisfy PDEs. This is why the paper's optimization problem (maximize expected utility) becomes a PDE (the [[hjb-equation|HJB Equation]]). You don't need to prove it — understand the connection.

- **Resource:** [[stochastic-calculus-for-finance-ii-steven-shreve|Stochastic Calculus for Finance II — Steven Shreve]], chapters 1-4. Standard textbook, written for finance people. Alternative: [[a-first-course-in-stochastic-calculus-louis-pierre-arguin|A First Course in Stochastic Calculus — Louis-Pierre Arguin]] for a gentler path. Supplement with [[mit-18-s096-topics-in-mathematics-with-applications-in-finance|MIT 18.S096 lectures 17-21]].
...
#### [[utility-theory|Utility Theory]] and [[risk-aversion|Risk Aversion]]
- Why maximize utility instead of raw profit (a dollar lost hurts more than a dollar gained helps)
- [[exponential-utility|Exponential Utility]] `U(x) = -exp(-γx)` — the paper's choice. γ = risk aversion parameter. High γ = hate risk. Low γ ≈ risk-neutral.
- [[reservation-price|Reservation Price]] (indifference prices) — the price at which you're equally happy buying or not. This is Definition 1 in [[avellaneda-stoikov|Avellaneda-Stoikov]] and the key concept.
...
- Discrete time → [[bellman-equation|Bellman Equation]] (recursion). Continuous time → [[hjb-equation|HJB Equation]] (PDE).

#### [[hjb-equation|HJB Equation]]s
...
- **Resource:** [[convex-optimization-boyd-and-vandenberghe|Convex Optimization — Boyd & Vandenberghe]], chapters 1-5 (free Stanford PDF) for the optimization foundations. [[algorithmic-and-high-frequency-trading-cartea-et-al|Algorithmic and High-Frequency Trading — Cartea et al.]], chapters on optimal control. Or [[continuous-time-stochastic-control-and-optimization-pham|Continuous-time Stochastic Control and Optimization — Pham]], chapters 1-3. For interview-style market-making cases that translate this theory into "where do I quote, given my position?" reasoning, work through the **MIT Quant Bible** (MIT Sloan Business Club PDF) section 6 cases (sports betting, country population, trade-or-tighten).
...
### 2.3 Reading the [[avellaneda-stoikov|Avellaneda-Stoikov]] Paper

With all prerequisites in place, work through it with pen and paper.

**Part 1: Sections 1-2**
- Derive value function (equation 3) yourself
- Derive [[reservation-price|Reservation Price]]s (equations 6, 7, 8) — algebra once you understand definitions
- Understand limit order model (section 2.4): how δa, δb control execution probability
- Work through trading intensity (section 2.5): [[poisson-process|Poisson Process]] rates + [[power-law-distribution|Power Law Distribution]] order sizes + price impact → equation 12
...
- Study simulation results: why [[inventory-risk|Inventory Risk]] management reduces variance at cost of slightly lower returns
...
- **[[spatial-arbitrage|Spatial Arbitrage]]** — same asset, different venue (Binance vs Coinbase, Uniswap vs SushiSwap). Latency-sensitive, highly competitive.
- **[[triangular-arbitrage|Triangular Arbitrage]]** — A→B→C→A within one exchange. Math-heavy, competitive.
- **[[cross-chain-arbitrage|Cross-Chain Arbitrage]]** — same asset across L1s/L2s. Bridge risk and finality delays are the challenge.
- **[[cex-dex-arbitrage|CEX-DEX Arbitrage]]** — price discrepancy between on-chain and off-chain. MEV territory.
- **[[funding-rate-arbitrage|Funding Rate Arbitrage]]** — perps vs spot, or perps across venues. More carry trade than pure arb.
- **[[statistical-arbitrage|Statistical Arbitrage]]** — not a riskless spread but a probabilistic edge from cross-venue signal extraction. This is where prediction markets come in.
...
## Unlocks
- [[level-3-hands-on-niche-markets|Level 3 — Hands-On Niche Markets]]
