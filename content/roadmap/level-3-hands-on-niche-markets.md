---
title: "Level 3 — Hands-On, Barely Profitable Quant in Niche Markets"
type: roadmap
level: 3
status: not-started
tags:
...
Linear regression, multiple regression, R², residuals, p-values. This is your primary tool for modeling "when prediction market probability moves X%, spot price moves Y%." [[logistic-regression|Logistic Regression]] for binary outcome modeling (prediction markets output probabilities).
...
- **[[autocorrelation|Autocorrelation]]** / partial autocorrelation — does today's return predict tomorrow's?
- **[[stationarity|Stationarity]]** — prices are non-stationary, but returns and spreads often are. Augmented Dickey-Fuller test to check.
- **[[arima|ARIMA]]** — AR, MA, ARMA, ARIMA — workhorses of time series. Model mean-reverting processes.
- **[[cointegration|Cointegration]]** — arguably the single most important concept for stat arb. Two prices can each be random walks, but their *spread* is mean-reverting. This is the math behind pairs trading and cross-venue arb. Engle-Granger method, Johansen test.
- **[[garch|GARCH]]** — volatility is predictable. Big moves follow big moves (volatility clustering). Model and forecast it.

- **Resource:** [[analysis-of-financial-time-series-ruey-tsay|Analysis of Financial Time Series — Ruey Tsay]], chapters 1-3 and 8. Or [[algorithmic-trading-ernie-chan|Algorithmic Trading — Ernie Chan]] for code-first approach with cointegration strategies.
...
p-values, confidence intervals, multiple testing correction (test 100 strategies → 5 look significant by chance). [[overfitting|Overfitting]] and data snooping are the graveyard of quant strategies. Know Bonferroni correction (conservative) and Benjamini-Hochberg (less conservative, controls false discovery rate). Use Newey-West standard errors for financial data — standard errors are wrong when returns are autocorrelated or heteroskedastic.

- **Resource:** [[all-of-statistics-larry-wasserman|All of Statistics — Larry Wasserman]], chapters 1-13. [[introduction-to-statistical-learning-islr|Introduction to Statistical Learning (ISLR)]] chapters 2-3. [[the-deflated-sharpe-ratio-bailey-and-lopez-de-prado|The Deflated Sharpe Ratio — Bailey & López de Prado]] for finance-specific multiple testing.
...
- The [[logarithmic-market-scoring-rule|Logarithmic Market Scoring Rule]] (LMSR) by Robin Hanson: the cost function tracks outstanding shares, the price function is a softmax (same function powering neural network classifiers). Prices always sum to 1 and lie in (0,1). Market maker's max loss is bounded at `b × ln(n)` where n is the number of outcomes.
...
This is your [[statistical-arbitrage|Statistical Arbitrage]] edge:
...
- [[market-impact|Market Impact]] (your order moves the price)
...
- **[[overfitting|Overfitting]]** — a strategy that fits historical data perfectly but has zero predictive power
...
- **Resource:** [[algorithmic-trading-ernie-chan|Algorithmic Trading — Ernie Chan]] — the best practical backtesting guide with code. [[advances-in-financial-machine-learning-lopez-de-prado|Advances in Financial Machine Learning — López de Prado]], chapters 7-8 on cross-validation for financial data.

### 3.4 Risk Management (non-negotiable)

#### [[position-sizing|Position Sizing]]
- **[[kelly-criterion|Kelly Criterion]]** — mathematically optimal bet size given your edge and variance. In practice use fractional Kelly (1/2 or 1/4) because real tails are fatter than your model assumes.
- Why even a positive-edge strategy ruins you with wrong sizing.

- **Resource:** [[a-man-for-all-markets-ed-thorp|A Man for All Markets — Ed Thorp]] for intuition. Original Kelly paper (short, readable).

#### Risk Metrics
- [[value-at-risk|Value at Risk]] (VaR) — nearly useless for crypto due to [[fat-tails|Fat Tails]]
- [[expected-shortfall|Expected Shortfall]] (Conditional VaR) — better, measures average loss in worst scenarios
- [[maximum-drawdown|Maximum Drawdown]] — what actually kills strategies in practice
- [[sharpe-ratio|Sharpe Ratio]], [[sortino-ratio|Sortino Ratio]] — risk-adjusted return measures

#### Crypto-Specific Risks
- Exchange counterparty risk (FTX)
- Smart contract risk (exploits, rug pulls)
- [[liquidity|Liquidity]] risk (order book is deep until it isn't)
...
- **Resource:** [[quantitative-risk-management-mcneil-frey-and-embrechts|Quantitative Risk Management — McNeil, Frey & Embrechts]], chapters 1-4, 7.
...
- **[[funding-rate-arbitrage|Funding Rate Arbitrage]] harvesting** — go long on a venue with negative funding, short on one with positive funding. Delta neutral. Capital-intensive but conceptually simple.
